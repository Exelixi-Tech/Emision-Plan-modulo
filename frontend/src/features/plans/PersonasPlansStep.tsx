...
function apiPlanToWizardPlan(p: PlanPer): Plan {
  return {
    ...
    priceNum: 0,
    tag: 'Personas',
    desc: 'Cobertura de póliza de personas / plan de salud y vida para los asegurados.',
    benefits: [
      'Servicio de cobertura integral',
...
export interface PersonasPlansStepProps {
  cramo?: number | string;
  centidad?: string;
  citem?: string;
  cproducto?: string;
...
export function PersonasPlansStep(props: PersonasPlansStepProps = {}) {
  const {
    funeral, selectedPlan, setSelectedPlan, setCategory,
    quote, quoteState, quoteError, metadataCanal,
  } = useWizardStore();
  ...
  // Parámetros de la vista para el API v2
  const urlParams = typeof window !== 'undefined' ? new URLSearchParams(window.location.search) : null;
  const storeMeta = (metadataCanal as Record<string, unknown> | null) ?? {};

  const viewCramo = props.cramo ?? (storeMeta.cramo != null ? String(storeMeta.cramo) : undefined) ?? urlParams?.get('cramo') ?? product.cramo ?? 7;
  const viewCentidad = props.centidad ?? (storeMeta.centidad != null ? String(storeMeta.centidad) : undefined) ?? urlParams?.get('centidad') ?? undefined;
  const viewCitem = props.citem ?? (storeMeta.citem != null ? String(storeMeta.citem) : (storeMeta.cproductor != null ? String(storeMeta.cproductor) : undefined)) ?? urlParams?.get('citem') ?? undefined;
  const viewCproducto = props.cproducto ?? (storeMeta.cproducto != null ? String(storeMeta.cproducto) : undefined) ?? urlParams?.get('cproducto') ?? undefined;

  // ── Carga de planes de personas v2 ─────────────────────────────────────────
  useEffect(() => {
    let cancelled = false;
    ...
    personApiV2.planes({
      cramo: viewCramo,
      centidad: viewCentidad,
      citem: viewCitem,
      cproducto: viewCproducto,
    })
      .then((res) => {
        if (cancelled) return;
      ...
      .catch((err) => {
        console.error('Error cargando planes personas v2', err);
        if (cancelled) return;
        setPlansError(true);
        setApiPlans([]);
      })
  ...
  }, [viewCramo, viewCentidad, viewCitem, viewCproducto, setSelectedPlan]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setFrecLoading(true);
    getFrecuenciasByPlan(planCode, Number(viewCramo))
      .then((items) => {
        if (cancelled) return;
    ...
    return () => { cancelled = true; };
  }, [selectedPlan?.cplan, viewCramo, setFuneral, funeral.frecuencia]);
  ...
  const quoteSig = planCode
    ? `personasV2|${planCode}|${viewCramo}|A|${aseguradosListos
  ...
  }, [quoteSig, planCode, viewCramo, aseguradosListos]);
    ...
    <div className="animate-fade-in space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap -mt-2">
        <p className="text-slate-500 text-sm leading-relaxed max-w-md">
          Selecciona el plan que mejor se ajuste a tu grupo familiar.
        </p>
